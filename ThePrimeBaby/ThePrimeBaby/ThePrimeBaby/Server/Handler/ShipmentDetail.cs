...
namespace ThePrimeBaby.Server.Handler
{
    public class ShipmentDetail
    {
        internal static void Register()
        {
            ...
            {
                QueryResultRows<Database.ShipmentDetail> shipmentDetail = Db.SQL<Database.ShipmentDetail>("SELECT c FROM ShipmentDetail c");
            ...
            Handle.POST("/ThePrimeBaby/AddConsignmentDetailByItemName/8", (Request r) =>
            {
                ...
                Database.Base.Item item = Db.SQL<Database.Base.Item>("SELECT i FROM Item i WHERE i.Name = ?",Attributes[0]).First;
                Database.Shipment shipment = Db.SQL<Database.Shipment>("SELECT c FROM Shipment c WHERE c.ID = ?", Convert.ToInt32(Attributes[1])).First;
                if (shipment == null)
                {
